...
execute as @s if data entity @s {data:{setup:0b}} at @s run function ancient_rituals:ruined_light_alter/setup
...
execute as @s at @s unless block ~ ~-1 ~ minecraft:lectern[has_book=true]{Book:{tag:{author:"Unknown",title:"Ancient Rituals"}}} run function ancient_rituals:ruined_light_alter/book_taken
